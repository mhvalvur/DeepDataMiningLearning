...
from hf_utils import WEIGHTS_NAME, Config, cached_path, hf_bucket_url, is_remote_url, load_checkpoint, img_tensorize, get_data
...
def build_backbone(cfg):
    input_shape = ShapeSpec(channels=len(cfg.MODEL.PIXEL_MEAN)) #(channels=3, height=None, width=None, stride=None
    ...
    )#7x7 conv2d in_channels=3, out_channels=64
    freeze_at = cfg.BACKBONE.FREEZE_AT #2

    if freeze_at >= 1:
        for p in stem.parameters():
            p.requires_grad = False

    out_features = cfg.RESNETS.OUT_FEATURES #['res4']
    depth = cfg.RESNETS.DEPTH #101
    num_groups = cfg.RESNETS.NUM_GROUPS #1
    width_per_group = cfg.RESNETS.WIDTH_PER_GROUP #64
    bottleneck_channels = num_groups * width_per_group #64
    in_channels = cfg.RESNETS.STEM_OUT_CHANNELS #64
    out_channels = cfg.RESNETS.RES2_OUT_CHANNELS #256
    stride_in_1x1 = cfg.RESNETS.STRIDE_IN_1X1 #True
    res5_dilation = cfg.RESNETS.RES5_DILATION #1
...
class GeneralizedRCNN(nn.Module):
    def __init__(self, cfg):
        ...
        self.device = torch.device(cfg.MODEL.DEVICE) #'cpu'
    ...
    def inference(
        ...
        **kwargs,
        ...
        if image_shapes is not None and scales_yx is not None:
            original_sizes = image_shapes * scales_yx #tensor([[ 800, 1199]]) * tensor([[0.5337, 0.5338]]) => tensor([[427., 640.]])

        features = self.backbone(images)#key 'res4' [1, 1024, 50, 75]

        # generate proposals if none are available
        if proposals is None:
            proposal_boxes, _ = self.proposal_generator(images, image_shapes, features, gt_boxes) #list of tensor [157, 4]
        else:
        ...
        boxes, classes, class_probs, attrs, attr_probs, roi_features = self.roi_outputs(
            obj_logits=obj_logits,#[157, 1601]
            attr_logits=attr_logits, #[157, 401]
            box_deltas=box_deltas, #[157, 6400]
            pred_boxes=proposal_boxes,#list of tensor [157, 4]
            features=feature_pooled, #[157, 2048]
            sizes=image_shapes, #[1, 2] tensor([[ 800, 1199]])
            scales=scales_yx,
        )
        ...
        preds_per_image = torch.tensor([p.size(0) for p in boxes]) #36
        boxes = pad_list_tensors(boxes, preds_per_image, **subset_kwargs) #[1, 36, 4]
        classes = pad_list_tensors(classes, preds_per_image, **subset_kwargs) #[1,36]
        class_probs = pad_list_tensors(class_probs, preds_per_image, **subset_kwargs) #[1, 36]
        attrs = pad_list_tensors(attrs, preds_per_image, **subset_kwargs) #[1, 36]
        attr_probs = pad_list_tensors(attr_probs, preds_per_image, **subset_kwargs) #[1, 36]
        roi_features = pad_list_tensors(roi_features, preds_per_image, **subset_kwargs) #[1, 36, 2048]
...
class Preprocess:
    ...
    def __call__(self, images, single_image=False, return_tensors="pt"):
        with torch.no_grad():
            ...
            raw_sizes = torch.tensor([im.shape[:2] for im in images]) #[[427, 640]]
            images = self.aug(images) #[3, 800, 1199]
            # transpose images and convert to torch tensors
            # images = [torch.as_tensor(i.astype("float32")).permute(2, 0, 1).to(self.device) for i in images]
            # now normalize before pad to avoid useless arithmetic
            images = [self.normalizer(x) for x in images] #[3, 800, 1199]
...
def showarray(a, fmt="jpeg"):
    ...
    #img.save("output/test.jpeg")

from transformers import AutoImageProcessor, AutoModelForObjectDetection
def HF_inference(model, image):
    ...
    with torch.no_grad():
        ...
        outputs = model(images=inputs['pixel_values'], original_sizes=image.size[::-1]) # ['logits'], ['pred_boxes'](center_x, center_y, width, height)
...
if __name__ == "__main__":
    ...
    frcnn_cfg = Config.from_pretrained("unc-nlp/frcnn-vg-finetuned") #https://s3.amazonaws.com/models.huggingface.co/bert/unc-nlp/frcnn-vg-finetuned/config.yaml

    model = GeneralizedRCNN.from_pretrained("unc-nlp/frcnn-vg-finetuned", config=frcnn_cfg)

    image_processor = Preprocess(frcnn_cfg)

    url = "https://unsplash.com/photos/HwBAsSbPBDU/download?ixid=MnwxMjA3fDB8MXxzZWFyY2h8MzR8fGNhciUyMGluJTIwdGhlJTIwc3RyZWV0fGVufDB8MHx8fDE2Nzg5MDEwODg&force=true&w=640"
    image = Image.open(requests.get(url, stream=True).raw)

    # run frcnn
    images, sizes, scales_yx = image_processor(url) #images: [1, 3, 800, 1199] CHW normalized RGB, sizes=800, 1199, scales_yx=[[0.5337, 0.5338]]
    output_dict = model(
        images,
        sizes,
        scales_yx=scales_yx,
    ...
    #HF_inference(model, image)

    OBJ_URL = "https://raw.githubusercontent.com/airsplay/py-bottom-up-attention/master/demo/data/genome/1600-400-20/objects_vocab.txt"
    ATTR_URL = "https://raw.githubusercontent.com/airsplay/py-bottom-up-attention/master/demo/data/genome/1600-400-20/attributes_vocab.txt"
    #VQA_URL = "https://dl.fbaipublicfiles.com/pythia/data/answers_vqa.txt"
    objids = get_data(OBJ_URL)#1600
    attrids = get_data(ATTR_URL)#400
